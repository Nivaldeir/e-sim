...
const editDocumentGroupSchema = z.object({
  name: z.string().min(1, "Nome é obrigatório"),
  description: z.string().optional(),
});

type EditDocumentGroupFormValues = z.infer<typeof editDocumentGroupSchema>;

interface EditDocumentGroupModalData {
  groupId: string;
  initialName: string;
...
export function EditDocumentGroupModal({
  onClose,
  data,
}: ModalProps<EditDocumentGroupModalData>) {
  const form = useZodForm(editDocumentGroupSchema, {
    defaultValues: {
  ...
  const updateGroupMutation = api.documentGroup.update.useMutation({
    onSuccess: () => {
      data?.onSuccess();
      onClose();
    },
  ...
  const handleSubmit = async (values: EditDocumentGroupFormValues) => {
  ...
  return (
    <div className="p-6">
      <div className="mb-4">
        <h2 className="text-2xl font-semibold">Editar Grupo de Documentos</h2>
